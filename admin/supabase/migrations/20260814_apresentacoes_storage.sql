...
values ('pdfs', 'pdfs', true, 262144000, array['application/pdf']::text[])
...
create policy "Leitura publica de apresentacoes"
on storage.objects for select
to public
using (bucket_id = 'pdfs' and (storage.foldername(name))[1] = 'apresentacoes');
...
to authenticated
with check (bucket_id = 'pdfs' and (storage.foldername(name))[1] = 'apresentacoes');
