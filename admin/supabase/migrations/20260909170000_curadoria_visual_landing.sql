alter table public.cliente_empreendimentos
  add column if not exists imagem_ids_selecionadas jsonb not null default '[]'::jsonb,
  add column if not exists landing_layout text not null default 'automatico';

alter table public.cliente_empreendimentos
  drop constraint if exists cliente_empreendimentos_landing_layout_check;
alter table public.cliente_empreendimentos
  add constraint cliente_empreendimentos_landing_layout_check
  check (landing_layout in ('automatico','editorial','imersivo','investidor'));
...
returns jsonb language sql stable security definer set search_path=public,pg_temp as $$
 select jsonb_build_object(
  ...
  'oportunidades',coalesce(jsonb_agg(jsonb_build_object(
   ...
   'layout',case when ce.landing_layout <> 'automatico' then ce.landing_layout when (ce.ordem-1)%3=0 then 'editorial' when (ce.ordem-1)%3=1 then 'imersivo' else 'investidor' end,
   ...
   'caracteristicas',case when ce.exibir_especificacoes or ce.exibir_descricao or ce.exibir_investimento or ce.exibir_fluxo then e.caracteristicas end,
   ...
   'imagens',case when ce.exibir_imagens then coalesce((select jsonb_agg(jsonb_build_object('id',ei.id,'titulo',ei.titulo,'categoria',ei.categoria,'storage_path',ei.storage_path,'url',case when ei.storage_path is null then ei.url end) order by ord.pos) from jsonb_array_elements_text(ce.imagem_ids_selecionadas) with ordinality ord(id,pos) join public.empreendimento_imagens ei on ei.id::text=ord.id where ei.empreendimento_id=e.id limit 10),'[]'::jsonb) else '[]'::jsonb end,
