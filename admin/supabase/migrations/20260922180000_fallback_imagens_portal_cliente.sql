-- Corrige o caso em que uma seleção antiga possui exibir_imagens=true,
-- mas imagem_ids_selecionadas ainda está vazio.
-- Nessa situação, a apresentação usa automaticamente a galeria do empreendimento.
...
as $$
  select jsonb_build_object(
    'cliente', jsonb_build_object(
      'nome', c.nome,
      'objetivo', c.objetivo,
      'modo', c.modo_apresentacao,
      'horizonte', c.horizonte_investimento,
      'cidade', c.cidade
    ),
    'oportunidades', coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'status', e.status,
          'layout', case
            when ce.landing_layout <> 'automatico' then ce.landing_layout
            when (ce.ordem - 1) % 3 = 0 then 'editorial'
            when (ce.ordem - 1) % 3 = 1 then 'imersivo'
            else 'investidor'
          end,
          ...
          'caracteristicas', case
            when ce.exibir_especificacoes
              or ce.exibir_descricao
              or ce.exibir_investimento
              or ce.exibir_fluxo
            then e.caracteristicas
          end,
          'diferenciais', case when ce.exibir_descricao then e.diferenciais end,
          'lazer', case when ce.exibir_descricao then e.lazer end,
          ...
          'permitir_proposta', ce.permitir_proposta,
          'imagens', case
            when ce.exibir_imagens then coalesce(
              nullif(
                (
                  select jsonb_agg(
                    jsonb_build_object(
          ...
          'plantas', case
            when ce.exibir_imagens or ce.exibir_especificacoes then coalesce(
              (
                select jsonb_agg(
                  jsonb_build_object(
                    'id', pu.id,
                    'titulo', pu.titulo,
                    'tipo', pu.tipo,
                    'tipologia', pu.tipologia,
                    'storage_path', pu.storage_path
                  )
                  order by pu.ordem, pu.created_at
                )
                from public.plantas_unidades pu
                where pu.empreendimento_id = e.id
              ),
              '[]'::jsonb
            )
            else '[]'::jsonb
  ...
  left join public.cliente_empreendimentos ce
    on ce.cliente_id = c.id
   and ce.visivel
  left join public.empreendimentos e
    on e.id = ce.empreendimento_id
