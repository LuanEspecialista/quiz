...
export async function refreshExchangeRate(): Promise<ExchangeRate | null> {
  ...
  if (!edge.error && !(edge.data as any)?.error) {
    const payload = edge.data as any;
    const updatedValue = Number(payload?.cotacao_venda);
    if (Number.isFinite(updatedValue) && updatedValue > 0) {
      const rate: ExchangeRate = {
        ...
        official: Boolean(payload?.oficial),
        notice: payload?.aviso || null,
      };
      remember(rate);
      return rate;
    }
    const rate = await getExchangeRate();
    if (rate) return { ...rate, provider: payload?.fonte || null, official: Boolean(payload?.oficial), notice: payload?.aviso || null };
  }
  throw new Error("Não foi possível consultar uma nova cotação agora. A última cotação válida continua em uso.");
}
