...
import { Building2, CalendarDays, Landmark, LogOut, MapPin, Send, UserRound, WalletCards, X } from "lucide-react";
...
export default function ClientPortal({ userName }: { userName?: string }) {
  ...
  return <main style={{ minHeight: "100vh", background: "#09090b", color: "#f4f4f5", padding: "clamp(20px,4vw,56px)" }}><div style={{ maxWidth: 1180, margin: "0 auto" }}>
    <header style={{ display: "flex", justifyContent: "space-between", gap: 18, flexWrap: "wrap", marginBottom: 32 }}><div><a href="/" style={{ color: "#d7ab63", textDecoration: "none", fontWeight: 800, fontSize: 12, letterSpacing: ".12em" }}>LUAN ESPECIALISTA</a><h1 style={{ fontSize: "clamp(28px,5vw,48px)", margin: "10px 0 8px" }}>Bem-vindo{firstName ? `, ${firstName}` : ""}.</h1><p style={{ color: "#a1a1aa", margin: 0 }}>Uma curadoria pessoal de oportunidades selecionadas para você.</p></div><div style={{display:"flex",gap:8}}><button onClick={()=>setAccountOpen(true)} style={{ border: "1px solid #5a4828", background: "#18140e", color: "#edcf91", borderRadius: 8, padding: "10px 13px", cursor: "pointer" }}><UserRound size={16}/> Minha conta</button><button onClick={() => void supabase.auth.signOut()} style={{ border: "1px solid #3f3f46", background: "#18181b", color: "#d4d4d8", borderRadius: 8, padding: "10px 13px", cursor: "pointer" }}><LogOut size={16} /> Sair</button></div></header>
    ...
    {accountOpen&&<div role="dialog" aria-modal="true" style={{position:"fixed",inset:0,zIndex:1100,background:"#000d",padding:18,overflow:"auto"}}><div style={{maxWidth:950,margin:"20px auto"}}><button onClick={()=>setAccountOpen(false)} style={{float:"right",position:"relative",zIndex:2}}><X size={18}/></button><MinhaConta compact/></div></div>}
