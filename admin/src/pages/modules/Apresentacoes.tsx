...
export default function Apresentacoes() {
  ...
  async function uploadPdf() {
    ...
    try {
      const upload = await supabase.storage.from(BUCKET).upload(path, file, { upsert: true, contentType: "application/pdf", cacheControl: "0" });
  ...
  function present(item: Empreendimento) {
    ...
    const separator = presentation.pdf_url.includes("?") ? "&" : "?";
    window.open(`${presentation.pdf_url}${separator}v=${encodeURIComponent(presentation.updated_at || Date.now())}`, "_blank", "noopener,noreferrer");
  }
  ...
  return <section style={{ maxWidth: 1240, margin: "0 auto", color: "#e4e4e7" }}>
    ...
    {editing && <div role="dialog" aria-modal="true" aria-labelledby="presentation-modal-title" style={overlayStyle} onMouseDown={(event) => { if (event.target === event.currentTarget) setEditing(null); }}>
      <div style={modalStyle}>
        ...
        <label style={labelStyle}>Novo PDF<input type="file" accept="application/pdf,.pdf" onChange={(event) => setFile(event.target.files?.[0] || null)} style={{ color: "#d4d4d8", padding: "10px 0" }} /></label>
