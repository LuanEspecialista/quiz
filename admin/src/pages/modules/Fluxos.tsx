...
import { isUnitAvailable, parseStandardTypology, readCommercialFlow } from "@/lib/realEstateStandard";
...
const dateLabel = (value: Date) => value.toLocaleDateString("pt-BR", { month: "short", year: "numeric" }).replace(" de ", "/");
...
function FlowNegotiator({ unit, availableCapital, defaultCdiRate, clientId, now }: { unit: Unidade; availableCapital: number; defaultCdiRate: number; clientId?: string; now: Date }) {
  ...
  const officialMonths = Math.max(1, officialDeliveryMonths || Number(raw.meses_ate_chaves) || Number(raw.parcelas_antes_chaves) || Number(raw.quantidade_parcelas_ate_chaves) || analysis.months || 60);
  const officialBalloonCount = Math.max(0, analysis.balloonCount || Number(raw.quantidade_baloes) || Number(raw.numero_baloes) || Number(raw.quantidade_reforcos) || Number(commercial.baloes_por_ano) * Math.max(1,Math.floor((analysis.months||60)/12)) || 0);
  const [months, setMonths] = useState(officialMonths);
  const [balloonCount, setBalloonCount] = useState(officialBalloonCount);
  ...
  const entryDefault = Math.min(preTarget, firstPositive(raw.ato, raw.entrada, raw.ato_valor, raw.entrada_valor, price * .1));
  const installmentDefault = Math.min(Math.max(0,preTarget-entryDefault)/months, firstPositive(raw.parcela_mensal, raw.valor_parcela, raw.parcela, raw.mensais_obra_val, raw.parcela_valor, raw.parcela_pre_chaves_valor) || Math.min(2500, Math.max(0, preTarget-entryDefault) / months));
  const postDefault = Math.max(0, price - preTarget);
  const keysDefault = Math.min(Math.max(0,preTarget-entryDefault-installmentDefault*months),firstPositive(raw.chaves, raw.valor_chaves, raw.parcela_chaves, raw.chaves_valor));
  ...
  const storedAppreciation = n(unit.empreendimentos?.valorizacao_aa);
  const [appreciation, setAppreciation] = useState(storedAppreciation);
  const [appreciationConfirmed, setAppreciationConfirmed] = useState(storedAppreciation !== 0);
  const [appreciationLocked, setAppreciationLocked] = useState(false);
  const [appreciationScenario, setAppreciationScenario] = useState<Cenario>("base");
  ...
  const [versionType, setVersionType] = useState<"proposta_luan" | "contraproposta_construtora" | "condicao_final">("proposta_luan");
  const [assetType, setAssetType] = useState<"nenhum" | "veiculo" | "imovel" | "terreno" | "consorcio" | "outro">("nenhum");
  const [assetMarketValue, setAssetMarketValue] = useState(0);
  const [assetAcceptedValue, setAssetAcceptedValue] = useState(0);
  ...
  async function saveSimulation() {
    ...
    const payload = { unidade_id: unit.id, cliente_id:clientId||null, nome: `${unit.empreendimentos?.nome || "Unidade"} · ${unit.codigo_unidade || unit.numero_unidade || "simulação"}`, cenario: "base", valores: { ...solved, entrada_atos: entryStages }, travas: locks, premissas: { tipo_versao: versionType, fluxo_oficial: { entrada: entryDefault, parcela: installmentDefault, balao: officialBalloonCount ? remainingForBalloons / officialBalloonCount : 0, quantidade_parcelas: officialMonths, quantidade_baloes: officialBalloonCount, percentual_ate_chaves: price > 0 ? preTarget / price * 100 : 0, entrega: deliveryRaw(unit.empreendimentos) || null }, bem_negociacao: assetType === "nenhum" ? null : { tipo: assetType, valor_mercado: assetMarketValue, valor_aceito: assetAcceptedValue, observacoes: assetNotes, sujeito_aprovacao: true }, analysisPurpose, monthlyRent, monthlyHoldingCost, appreciation, cdiRate, horizonYears, postKeysMonths, postKeysRate, postKeysMode, postKeysBalloonCount, postKeysBalloonValue, cashReserve, recurringIncome, passiveIncome, monthlyCommitments }, metricas: { tir:metrics.tir, roi:metrics.roi, vpl:metrics.vpl, cap_rate_bruto:metrics.capRateGross, cap_rate_liquido:metrics.capRateNet, cash_on_cash:metrics.cashOnCash, yield_on_cost:metrics.yieldOnCost, pico_capital:metrics.peakCapital }, cronograma: metrics.schedule.map((event) => ({ ...event, data_estimada: addMonths(now, event.month).toISOString().slice(0, 10) })), status: valid ? "valido" : shortage ? "proposta_construtora" : "incompleto", updated_at:new Date().toISOString() };
  ...
  const investmentReady = analysisPurpose !== "moradia" && appreciationConfirmed;
  const executiveMetrics = [["TIR líquida",!investmentReady?"Confirme a projeção":metrics.tir==null?"Dados insuficientes":`${metrics.tir.toLocaleString("pt-BR",{maximumFractionDigits:2})}% a.a.`],["VPL",investmentReady&&metrics.comparisonComplete?money(metrics.vpl):"Confirme valorização e CDI"],["ROI líquido",!investmentReady?"Confirme a projeção":metrics.roi==null?"Dados insuficientes":`${metrics.roi.toLocaleString("pt-BR",{maximumFractionDigits:2})}%`],["Cap rate líquido",analysisPurpose==="renda"&&investmentReady&&metrics.rentalComplete&&metrics.capRateNet!=null?`${metrics.capRateNet.toLocaleString("pt-BR",{maximumFractionDigits:2})}% a.a.`:"Informe aluguel e custos"],["Cash-on-cash",analysisPurpose==="renda"&&investmentReady&&metrics.rentalComplete&&metrics.cashOnCash!=null?`${metrics.cashOnCash.toLocaleString("pt-BR",{maximumFractionDigits:2})}% a.a.`:"Informe aluguel e custos"],["Pico de capital",money(metrics.peakCapital)],["Dívida na saída",investmentReady?money(metrics.remainingDebt):"Confirme a projeção"],["CDI líquido equivalente",investmentReady&&metrics.comparisonComplete?money(metrics.benchmark.net):"Confirme valorização e CDI"],["Venda líquida projetada",investmentReady?money(metrics.saleNet):"Confirme a projeção"]];
  ...
  return <section className="flow-negotiator" style={{ background:"#0b0b0d", border:"1px solid #29292e", borderRadius:9, padding:10, marginTop:10 }}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}><div><h3 style={{fontSize:15,margin:0}}>Simulador inteligente · {unit.empreendimentos?.nome}</h3><p style={{fontSize:12,color:"#a1a1aa",margin:"5px 0 7px"}}>Valor da unidade: {money(price)} · referência oficial: {dateLabel(now)} · chaves: {deliveryLabelPt(deliveryRaw(unit.empreendimentos)) || dateLabel(addMonths(now, officialMonths))}</p><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><label style={{fontSize:10,color:"#a1a1aa"}}>Meses até as chaves <input type="number" min="1" max="360" value={months} onChange={(event)=>setMonths(Math.max(1,Math.round(n(event.target.value))))} style={{...inputStyle,width:70,padding:5,marginLeft:4}}/></label><label style={{fontSize:10,color:"#a1a1aa"}}>Quantidade de balões <input type="number" min="0" max="60" value={balloonCount} onChange={(event)=>setBalloonCount(Math.max(0,Math.round(n(event.target.value))))} style={{...inputStyle,width:65,padding:5,marginLeft:4}}/></label></div></div><span style={{alignSelf:"start",padding:"5px 9px",borderRadius:99,border:`1px solid ${valid?"#166534":shortage?"#854d0e":"#7f1d1d"}`,color:valid?"#4ade80":shortage?"#fbbf24":"#f87171",fontSize:11}}>{valid?"Fluxo fecha":shortage?`Abaixo do mínimo em ${money(shortage)}`:`Ajuste os campos livres`}</span></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:7,marginTop:9,padding:9,border:"1px solid #3f3219",borderRadius:8,background:"#17130b"}}><div><small style={{color:"#a1a1aa"}}>Fluxo oficial importado</small><strong style={{display:"block",fontSize:12,marginTop:3}}>{officialMonths} mensais · {officialBalloonCount} balões · {(price > 0 ? preTarget / price * 100 : 0).toLocaleString("pt-BR",{maximumFractionDigits:2})}% até as chaves</strong></div><div><small style={{color:"#a1a1aa"}}>Fluxo em negociação</small><strong style={{display:"block",fontSize:12,marginTop:3}}>{months} mensais · {balloonCount} balões · {(price > 0 ? paidBeforeKeys / price * 100 : 0).toLocaleString("pt-BR",{maximumFractionDigits:2})}% até as chaves</strong></div><div><small style={{color:"#a1a1aa"}}>Classificação ao salvar</small><select value={versionType} onChange={(event)=>setVersionType(event.target.value as typeof versionType)} style={{...inputStyle,marginTop:3,padding:6}}><option value="proposta_luan">Proposta Luan</option><option value="contraproposta_construtora">Contraproposta da construtora</option><option value="condicao_final">Condição final acordada</option></select></div></div>
    <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap",marginTop:8,padding:"7px 9px",border:"1px solid #3f3219",borderRadius:8,background:"#17130d"}}><small style={{color:"#d6a94f",fontWeight:800}}>Projeção de valorização</small><strong style={{fontSize:11}}>{unit.codigo_unidade||unit.numero_unidade||"Unidade"}</strong>{(["conservador","base","otimista"] as Cenario[]).map(value=><button key={value} type="button" disabled={appreciationLocked||storedAppreciation===0} onClick={()=>{setAppreciationScenario(value);setAppreciation(annualRate(storedAppreciation,value));setAppreciationConfirmed(storedAppreciation!==0)}} style={{background:appreciationScenario===value?"#3a2d17":"transparent",border:"1px solid #4a402d",borderRadius:99,color:appreciationScenario===value?"#f5d58b":"#8b8b95",padding:"3px 7px",fontSize:9,cursor:"pointer",textTransform:"capitalize"}}>{value}</button>)}<input aria-label="Valorização anual projetada" type="number" min="-100" step=".1" disabled={appreciationLocked} value={appreciationConfirmed?appreciation:""} placeholder="% a.a." onChange={event=>{setAppreciation(n(event.target.value));setAppreciationConfirmed(event.target.value!=="")}} style={{...inputStyle,width:76,padding:"4px 6px",fontSize:10}}/><button type="button" disabled={!appreciationConfirmed} title={appreciationLocked?"Destravar projeção":"Travar projeção nesta sessão"} onClick={()=>setAppreciationLocked(value=>!value)} style={{background:"transparent",border:0,color:appreciationLocked?"#d6a94f":"#71717a",padding:2,cursor:"pointer"}}>{appreciationLocked?<Lock size={13}/>:<LockOpen size={13}/>}</button><small style={{color:"#71717a"}}>{storedAppreciation?"Base cadastrada: "+storedAppreciation+"% a.a.":"Sem taxa cadastrada"}{appreciationConfirmed?" · até as chaves: "+((Math.pow(1+appreciation/100,months/12)-1)*100).toLocaleString("pt-BR",{maximumFractionDigits:1})+"%":""}</small></div>
    ...
    <details style={{marginTop:10,border:"1px solid #27272a",borderRadius:8,padding:"9px 10px",background:"#101012"}}><summary style={{cursor:"pointer",fontSize:12,fontWeight:700,color:"#d4d4d8"}}>Bem na negociação / permuta <span style={{fontWeight:400,color:"#71717a"}}>(opcional)</span></summary><p style={{fontSize:11,color:"#a1a1aa",margin:"7px 0"}}>Registre separadamente o valor de mercado e o valor que a construtora aceita. A inclusão nunca significa aprovação automática.</p><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(175px,1fr))",gap:8}}><label style={{fontSize:11,color:"#a1a1aa"}}>Tipo<select value={assetType} onChange={(event)=>setAssetType(event.target.value as typeof assetType)} style={{...inputStyle,marginTop:5}}><option value="nenhum">Nenhum</option><option value="veiculo">Veículo</option><option value="imovel">Imóvel</option><option value="terreno">Terreno</option><option value="consorcio">Carta de consórcio</option><option value="outro">Outro</option></select></label>{assetType!=="nenhum"&&<><label style={{fontSize:11,color:"#a1a1aa"}}>Valor de mercado<CurrencyInput value={assetMarketValue} onChange={setAssetMarketValue} style={{...inputStyle,marginTop:5}}/></label><label style={{fontSize:11,color:"#a1a1aa"}}>Valor aceito no fluxo<CurrencyInput value={assetAcceptedValue} onChange={setAssetAcceptedValue} style={{...inputStyle,marginTop:5}}/></label><label style={{fontSize:11,color:"#a1a1aa"}}>Observações<input value={assetNotes} onChange={(event)=>setAssetNotes(event.target.value)} placeholder="Avaliação, documentação, condição" style={{...inputStyle,marginTop:5}}/></label></>}</div></details>
...
export function Fluxos({ initialUnitIds = [], initialUnits = [], initialClientId }: { initialUnitIds?: string[]; initialUnits?: Unidade[]; initialClientId?: string }) {
  ...
  const visibleUnits = useMemo(() => units.filter((unit) => {
    ...
    const available = isUnitAvailable(unit.status);
  ...
  return <div style={{ color: "#f4f4f5" }}>
    ...
    {chosenUnits.length === 0 ? <section style={{ ...card, marginTop: 16, minHeight: 210, display: "grid", placeItems: "center", textAlign: "center", color: "#71717a" }}><div><BarChart3 size={42} style={{ margin: "0 auto 12px" }} /><strong style={{ color: "#d4d4d8" }}>Tela pronta para uma nova apresentação</strong><p>Escolha uma ou mais unidades acima para abrir fluxo, diferenças e gráficos.</p></div></section> : <>
      <section style={{ ...card, marginTop: 16 }}><h2 style={{ fontSize: 15, marginTop: 0 }}>Resumo executivo das opções</h2><p style={{ color: "#a1a1aa", fontSize: 12 }}>Valores de tabela e condições oficiais cadastradas. “Não informado” permanece explícito; o sistema não completa dados comerciais por suposição.</p><div style={{overflowX:"auto"}}><table style={{width:"100%",minWidth:760,borderCollapse:"collapse",fontSize:12}}><thead><tr>{["Unidade","Preço","Área","R$/m²","Entrega","Meses restantes","Fluxo até chaves","Status da leitura"].map((label)=><th key={label} style={{textAlign:"left",padding:"8px 7px",borderBottom:"1px solid #3f3f46",color:"#a1a1aa",whiteSpace:"nowrap"}}>{label}</th>)}</tr></thead><tbody>{chosenUnits.map((unit)=>{const flow=analyzeFlow(unit,{entrada:0,parcela:0,balao:0});const area=n(unit.area_privativa);const delivery=deliveryRaw(unit.empreendimentos);return <tr key={unit.id}>{[`${unit.empreendimentos?.nome || "—"} · ${unit.codigo_unidade || unit.numero_unidade || "—"}`,money(n(unit.valor_tabela)),area?`${area.toLocaleString("pt-BR",{maximumFractionDigits:2})} m²`:"Não informada",area?money(n(unit.valor_tabela)/area):"Não informado",delivery?deliveryLabelPt(delivery):"Não informada",delivery?String(monthsToDelivery(now,delivery)):"Não informado",flow.preKeysPercent?`${flow.preKeysPercent.toLocaleString("pt-BR",{maximumFractionDigits:2})}% / ${(100-flow.preKeysPercent).toLocaleString("pt-BR",{maximumFractionDigits:2})}%`:"Não informado",flow.status==="incompleto"?"Dados oficiais incompletos":"Pronto para negociar"].map((value,index)=><td key={index} style={{padding:"9px 7px",borderBottom:"1px solid #27272a",color:index===7&&flow.status==="incompleto"?"#fbbf24":"#e4e4e7",whiteSpace:index===0?"normal":"nowrap"}}>{value}</td>)}</tr>})}</tbody></table></div></section>
