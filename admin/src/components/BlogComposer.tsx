...
export default function BlogComposer({ editing, setEditing, field, setMessage }: Props) {
  ...
  return <section style={{ gridColumn: "1/-1", display: "grid", gap: 12, padding: 13, border: "1px solid #4c4028", borderRadius: 9, background: "#12110f" }}>
    ...
    {sections.map((section, index) => <article key={section.id} style={{ padding: 11, border: "1px solid #383840", borderRadius: 8, background: "#151518", display: "grid", gap: 8 }}>
      ...
      {!["imagem", "galeria", "lista", "comparativo", "subtitulo", "video"].includes(section.tipo) && <textarea value={section.texto || ""} onChange={(event) => updateSection(index, { texto: event.target.value })} rows={section.tipo === "texto" || section.tipo === "imagem_texto" ? 5 : 2} placeholder={section.tipo === "dado" ? "Número, contexto e significado" : "Texto curto e objetivo"} style={field}/>}
      ...
      {(section.tipo === "imagem" || section.tipo === "imagem_texto" || section.tipo === "galeria") && <>
        <input value={section.sugestao_imagem || ""} onChange={(event) => updateSection(index, { sugestao_imagem: event.target.value })} placeholder="Orientação e termos para localizar a imagem" style={field}/>
        {section.tipo !== "galeria" && <>
          <select value={section.imagem_url || ""} onChange={(event) => { const image = editing.imagens.find((item) => item.url === event.target.value); updateSection(index, { imagem_url: event.target.value, imagem_alt: image?.alt || section.imagem_alt || "", legenda: image?.legenda || section.legenda || "" }); }} style={field}><option value="">Selecionar imagem enviada</option>{editing.imagens.map((image) => <option key={image.url} value={image.url}>{image.alt || "Imagem sem descrição"}</option>)}</select>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 7 }}>
            <select value={section.tamanho || "media"} onChange={(event) => updateSection(index, { tamanho: event.target.value as EditorialSection["tamanho"] })} style={field}><option value="pequena">Pequena</option><option value="media">Média</option><option value="ampla">Ampla</option></select>
            <select value={section.alinhamento || "esquerda"} onChange={(event) => updateSection(index, { alinhamento: event.target.value as EditorialSection["alinhamento"] })} style={field}><option value="esquerda">Esquerda</option><option value="direita">Direita</option><option value="centro">Centro</option></select>
            <select value={section.proporcao || "paisagem"} onChange={(event) => updateSection(index, { proporcao: event.target.value as EditorialSection["proporcao"] })} style={field}><option value="quadrada">Quadrada</option><option value="vertical">Vertical</option><option value="paisagem">Paisagem</option></select>
          </div>
          <input value={section.legenda || ""} onChange={(event) => updateSection(index, { legenda: event.target.value })} placeholder="Legenda opcional" style={field}/>
        </>}
        {section.tipo === "galeria" && <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{editing.imagens.map((image) => { const selected = (section.imagens || []).some((item) => item.url === image.url); return <label key={image.url} style={{ display: "flex", alignItems: "center", gap: 6, padding: 7, border: `1px solid ${selected ? "#8b692b" : "#393940"}`, borderRadius: 6, fontSize: 11 }}><input type="checkbox" checked={selected} onChange={(event) => updateSection(index, { imagens: event.target.checked ? [...(section.imagens || []), image] : (section.imagens || []).filter((item) => item.url !== image.url) })}/>{image.alt || "Imagem sem descrição"}</label> })}</div>}
      </>}
      {section.tipo === "video" && <><input value={section.video_url || ""} onChange={(event) => updateSection(index, { video_url: event.target.value })} placeholder="Link do YouTube ou Vimeo" style={field}/><input value={section.video_titulo || ""} onChange={(event) => updateSection(index, { video_titulo: event.target.value })} placeholder="Título acessível do vídeo" style={field}/></>}
      {(section.tipo === "cta" || section.tipo === "pergunta") && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}><input value={section.acao_rotulo || ""} onChange={(event) => updateSection(index, { acao_rotulo: event.target.value })} placeholder="Texto do botão" style={field}/><input value={section.acao_url || ""} onChange={(event) => updateSection(index, { acao_url: event.target.value })} placeholder="Link opcional" style={field}/></div>}
    </article>)}
